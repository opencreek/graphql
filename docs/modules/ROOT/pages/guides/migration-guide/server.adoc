...
To achieve the same behaviour with the new `@opencreek/neo4j-graphql` library, the GraphQL schema has to be extended instead. Note, no additional configuration or parameters need to be passed to the `getSchema()` function.
...
In `@opencreek/neo4j-graphql`, this has now become:
