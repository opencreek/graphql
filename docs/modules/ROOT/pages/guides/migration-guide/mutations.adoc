...
This section will walk through each operation available through GraphQL Mutations, and how to migrate each from `neo4j-graphql-js` to `@opencreek/neo4j-graphql`.

The examples in this section will be based off the following type definitions (which have been migrated over to `@opencreek/neo4j-graphql` syntax):
...
> Note that xref::mutations/index.adoc[Mutations] in `@opencreek/neo4j-graphql` are incredibly powerful, and it is well worthwhile reading about them in full. You might find that you can collapse multiple current mutations down into one!

== Creating

For creating nodes, the input arguments of `neo4j-graphql` have been moved inside an `input` argument in `@opencreek/neo4j-graphql`.
...
Looks like the following using `@opencreek/neo4j-graphql`:
...
This has all changed in `@opencreek/neo4j-graphql`, with a `where` argument to select the node, and then an `update` argument (amongst many others) to select what to update.
...
In `@opencreek/neo4j-graphql`, this is achieved by specifying the `connect` argument on either the `updateMovies` or `updateActors` Mutation.
...
Would become the following using `@opencreek/neo4j-graphql`:
...
Note, there are many ways to achieve the same goal using the powerful Mutation ability of `@opencreek/neo4j-graphql`, so do what feels best for your data!
...
In `@opencreek/neo4j-graphql`, this is achieved by specifying the `disconnect` argument on either the `updateMovies` or `updateActors` Mutation.
