...
In `@opencreek/neo4j-graphql`, the ethos has been to simplify the top-level arguments:
...
Is now changed to the following using `@opencreek/neo4j-graphql`:
...
[cols="1,1"]
|===
|`neo4j-graphql-js` |`@opencreek/neo4j-graphql`
...
Sorting has changed somewhat in `@opencreek/neo4j-graphql`. For the example being used in this page, the `_MovieOrdering` type in `neo4j-graphql-js` was an enum which looked like the following:
...
In `@opencreek/neo4j-graphql`, the sorting type `MovieSort` has become an input type with each field as an enum, like follows:
...
Using `@opencreek/neo4j-graphql`, this will now be:
