...
This page describes the basics on how to use `@opencreek/neo4j-graphql` as part of a https://nextjs.org[Next.js] application.
...
At this point it's time to add the dependencies needed to run `@opencreek/neo4j-graphql` inside Next.js.
...
== Start the server
That is all that needs to be done for `@opencreek/neo4j-graphql` to work in a Next.js application. The dev server starts wtih `yarn dev` and Apollo Studio should be loaded on http://localhost:3000/api/graphql and the front-end part of the Next.js application should be able to query the API.
