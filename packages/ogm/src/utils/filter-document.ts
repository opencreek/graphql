...
export function filterDocument(typeDefs: Neo4jGraphQLConstructor["typeDefs"], excludeAuth?: boolean): DocumentNode {
            ...
            return [
                ...res,
                {
                    ...
                    directives: def.directives?.filter((x) => {
                        if (x.name.value === "authorization" && excludeAuth === false) return true;
