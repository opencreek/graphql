        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                CALL {
                    ...
                    WITH *
                    WHERE apoc.util.validatePredicate(NOT ((create_var6.fieldA IS NULL OR any(create_var11 IN [\\"role-A\\"] WHERE any(create_var10 IN $auth.roles WHERE create_var10 = create_var11))) AND (create_var6.fieldB IS NULL OR any(create_var13 IN [\\"role-B\\"] WHERE any(create_var12 IN $auth.roles WHERE create_var12 = create_var13)))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
                    RETURN collect(NULL) AS create_var14
                }
                WITH *
                WHERE apoc.util.validatePredicate(NOT (any(create_var16 IN [\\"admin\\"] WHERE any(create_var15 IN $auth.roles WHERE create_var15 = create_var16))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
                RETURN create_this0
            }
            CALL {
                WITH create_this0
                MATCH (create_this0)<-[create_this1:ACTED_IN]-(create_this2:\`Actor\`)
                WHERE apoc.util.validatePredicate(NOT ((create_this2.nodeCreatedBy IS NOT NULL AND create_this2.nodeCreatedBy = $create_param0)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
