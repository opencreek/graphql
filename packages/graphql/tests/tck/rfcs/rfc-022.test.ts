        ...
        test("Nested query", async () => {
            ...
            expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
                ...
                CALL {
                    WITH this
                    MATCH (this)<-[this0:ACTED_IN]-(this1:\`Person\`)
                    WHERE (this1.name = $param1 AND (($isAuthenticated = true AND ($param3 IS NOT NULL AND this1.name = $param3)) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (($jwt.test IS NOT NULL AND this1.name = $jwt.test) AND $param5 IN $jwt.roles)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])))
                    WITH this1 { .name } AS this1
                    RETURN collect(this1) AS var2
                }
                RETURN this { .title, actors: var2 } AS this"
