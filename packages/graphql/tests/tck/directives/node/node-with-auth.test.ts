...
describe("Node Directive", () => {
    ...
    test("Admin Deletes Post", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WITH *
            WHERE ((creatorCount <> 0 AND this0.id = $param0) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND $param2 IN $jwt.roles), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
            DETACH DELETE this"
        `);
