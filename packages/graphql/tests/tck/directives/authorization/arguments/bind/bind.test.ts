...
describe("Cypher Auth Allow", () => {
    ...
    test("Create Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                WITH *
                WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND create_this1.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
                RETURN create_this1
            }
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                CALL {
                    ...
                    WITH *
                    WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND create_this13.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
                    WITH create_this5
                    CALL {
                    	WITH create_this5
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
            	...
            	CALL {
            		...
            		WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this_posts0_creator0.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
    ...
    test("Disconnect Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WITH *, count(authorization_this0) AS creatorCount
            WITH *
            WHERE (apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND authorization_this0.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this_disconnect_creator0.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
