...
describe("Cypher Auth Allow", () => {
    ...
    test("Read Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            "MATCH (this:\`User\`)
            WITH *
            WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
    ...
    test("Read Relationship", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                WITH *
                WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND this2.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
    ...
    test("Read Relationship & Protected Field", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WITH *
            WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND this0.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
            CALL {
                WITH this
                MATCH (this)<-[this1:HAS_POST]-(this2:\`User\`)
                WHERE (apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this2.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this2.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
    ...
    test("Read Two Relationships", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            "MATCH (this:\`User\`)
            WITH *
            WHERE (this.id = $param0 AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
            CALL {
                WITH this
                MATCH (this)-[this0:HAS_POST]->(this1:\`Post\`)
                OPTIONAL MATCH (this1)<-[:HAS_POST]-(this2:\`User\`)
                WITH *, count(this2) AS creatorCount
                WITH *
                WHERE (this1.id = $param4 AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND this2.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
                CALL {
                    WITH this1
                    ...
                    WITH *
                    WHERE (this4.id = $param5 AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND this5.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
    ...
    test("Nested Update Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WITH *, count(this0) AS creatorCount
            WITH *
            WHERE (this.id = $param0 AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND this0.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
            WITH this
            CALL {
            	WITH this
            	MATCH (this)<-[this_has_post0_relationship:HAS_POST]-(this_creator0:User)
            	WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this_creator0.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
    ...
    test("Nested Delete Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WHERE this_posts0.id = $this_deleteUsers_args_delete_posts0_where_this_posts0param0 AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND authorization_this0.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
    ...
    test("Disconnect Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WITH *, count(authorization_this0) AS creatorCount
            WHERE this_disconnect_posts0.id = $updateUsers_args_disconnect_posts0_where_Post_this_disconnect_posts0param0 AND (apoc.util.validatePredicate(NOT ($isAuthenticated = true AND this.id = coalesce($jwt.sub, $jwtDefault)), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND authorization_this0.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
            CALL {
    ...
    test("Nested Disconnect Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WHERE (apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND authorization_this0.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND authorization_this1.id = coalesce($jwt.sub, $jwtDefault))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
            CALL {
