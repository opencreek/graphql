...
describe("Field Level Aggregations", () => {
    ...
    test("Count aggregation with auth in aggregated node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            "MATCH (this:Actor)
            RETURN this { .name, moviesAggregate: { count: head(apoc.cypher.runFirstColumn(\\"MATCH (this)-[r:ACTED_IN]->(n:Movie)     CALL apoc.util.validate(NOT(apoc.util.validatePredicate(NOT($auth.isAuthenticated = true), \\\\\\"@opencreek/neo4j-graphql/UNAUTHENTICATED\\\\\\", [0])), \\\\\\"@opencreek/neo4j-graphql/FORBIDDEN\\\\\\", [0]) RETURN COUNT(n)\\", { auth: $auth, this: this })) } } as this"
        `);
