...
describe("#601", () => {
    ...
    test("Example 1", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL apoc.util.validate(NOT(ANY(r IN [\\"view\\"] WHERE ANY(rr IN $auth.roles WHERE r = rr))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
            RETURN this { documents: [ (this)-[:REQUIRES]->(this_documents:Document)  WHERE apoc.util.validatePredicate(NOT(ANY(r IN [\\"view\\"] WHERE ANY(rr IN $auth.roles WHERE r = rr))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]) | this_documents { customerContactConnection: apoc.cypher.runFirstColumn(\\"CALL {
            WITH this_documents
