...
describe("https://github.com/neo4j/graphql/issues/1132", () => {
    test("Auth CONNECT rules checked against correct property", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
            	...
            	CALL apoc.util.validate(NOT(this.id IS NOT NULL AND this.id = $thisSource1_allow_auth_allow0_id), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
    ...
    test("Auth DISCONNECT rules checked against correct property", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL apoc.util.validate(NOT(this.id IS NOT NULL AND this.id = $thisSource0_allow_auth_allow0_id), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
