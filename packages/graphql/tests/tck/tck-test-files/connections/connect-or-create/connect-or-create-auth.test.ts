    ...
    describe("Update -> connectOrCreate", () => {
        ...
        test("Create with createOrConnect and allow in auth", async () => {
            ...
            expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
                ...
                CALL {
                	WITH this
                	CALL apoc.util.validate(NOT(this_connectOrCreate_genres0.name IS NOT NULL AND this_connectOrCreate_genres0.name = $this_connectOrCreate_genres0Genre_allow_auth_allow0_name), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
