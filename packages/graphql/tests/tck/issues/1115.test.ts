    ...
    test("multiple connectOrCreate operations with auth", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                WITH *
                WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND $this_connectOrCreate_children_param3 IN $jwt.roles), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
                RETURN COUNT(*) AS _
            }
            WITH this
            CALL {
                ...
                WITH *
                WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND $this_connectOrCreate_children_param7 IN $jwt.roles), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
                RETURN COUNT(*) AS _
            }
            WITH *
