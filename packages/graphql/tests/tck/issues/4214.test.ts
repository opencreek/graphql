...
describe("https://github.com/neo4j/graphql/issues/4214", () => {
    ...
    test("should include checks for auth jwt param is not null", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WITH *
            WHERE (apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (($jwt.roles IS NOT NULL AND $authorization_0_0_0_0_after_param2 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_0_0_0_0_after_param3 IN $jwt.roles)) AND authorization_0_0_0_0_after_var0 = true), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (($jwt.roles IS NOT NULL AND $authorization_0_0_0_0_after_param4 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_0_0_0_0_after_param5 IN $jwt.roles)) AND (storeCount <> 0 AND ($jwt.store IS NOT NULL AND authorization_0_0_0_0_after_this1.id = $jwt.store))), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0]))
            ...
            CALL {
            	...
            	WHERE apoc.util.validatePredicate(NOT (c <= 1), '@opencreek/neo4j-graphql/RELATIONSHIP-REQUIREDTransactionItem.transaction must be less than or equal to one', [0])
            ...
            WITH *
            WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (($jwt.roles IS NOT NULL AND $authorization_0_0_0_0_after_param2 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_0_0_0_0_after_param3 IN $jwt.roles)) AND authorization_0_0_0_0_after_var0 = true), \\"@opencreek/neo4j-graphql/FORBIDDEN\\", [0])
            RETURN this0
            }
            CALL {
                WITH this0
